...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        计算 margin 和损失
        :param x: 对抗样本输入
        :param y: 正确的标签分布（或目标分布）
        """

        # 获取 logits 并计算损失
        logits = self.predict(x)  # [batch_size, 25]
        xent = test_soft_batch()(logits, y)

        # 处理 logits：取前 24 个值并分组
        logits_valid = logits[:, :24]  # 取前 24 个值
        logits_grouped = logits_valid.view(logits_valid.shape[0], -1, 4)  # 每 4 个值分为一组
        logits_softmax = 3 * torch.softmax(logits_grouped, dim=2)  # 应用 Softmax
        logits_consider = logits_softmax[:, :, :3]  # 取前三个值

        # 处理 y：取前 24 个值并分组
        y_valid = y[:, :24]  # 取前 24 个值
        y_grouped = y_valid.view(y_valid.shape[0], -1, 4)  # 每 4 个值分为一组
        y_consider = y_grouped[:, :, :3]  # 取前三个值

        # 找出 y_consider 中每组最大值及其索引
        y_max, y_max_idx = y_consider.max(dim=2)  # 最大值和索引 [batch_size, 6]

        # 根据索引从 logits_consider 中取对应项
        u = torch.arange(logits_consider.shape[0]).unsqueeze(-1)  # 样本索引 [batch_size, 1]
        logits_max = logits_consider[u, torch.arange(6), y_max_idx]  # [batch_size, 6]

        # 计算每组差值
        group_margin = logits_max - y_max  # [batch_size, 6]

        # margin 为六组差值的总和
        margin = group_margin.sum(dim=1)  # [batch_size]
    ...
    def perturb(self, x, y=None):
        ...
        if not self.targeted:
            ...
            acc = test_soft_batch()(self.predict(x), y) < 0.125
        else:
            acc = self.predict(x).max(1)[1] != y
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                adv_curr = torch.round(adv_curr)
                output_curr = self.predict(adv_curr)
                if not self.targeted:
                    # acc_curr = output_curr.max(1)[1] == y_to_fool
                    acc_curr = test_soft_batch()(output_curr, y_to_fool) < 0.125
                else:
                    acc_curr = output_curr.max(1)[1] != y_to_fool
