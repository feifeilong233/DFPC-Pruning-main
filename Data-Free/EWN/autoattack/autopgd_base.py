...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if not self.is_tf_model:
            if self.loss == 'ce':
                criterion_indiv = test_soft_batch()
        ...
        acc = test_soft_batch()(logits.detach(), y) < 0.125
        acc_steps[0] = acc + 0
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(torch.min(torch.max(x_adv_1,
                        x - self.eps), x + self.eps), 0.0, 12.0)
                    ...
                    x_adv_1 = torch.clamp(x + self.normalize(x_adv_1 - x
                        ...
                        L2_norm(x_adv_1 - x, keepdim=True)), 0.0, 12.0)
            ...
            pred = test_soft_batch()(logits.detach(), y) < 0.125
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        if self.loss != 'ce-targeted':
            acc = test_soft_batch()(y_pred, y) < 0.125
        else:
            acc = y_pred != y
