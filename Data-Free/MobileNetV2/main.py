...
def main_worker(gpu, args):
    ...
    if args.dataset in ['cifar100']:
        pretrained_checkpoint_path = './pretrained_checkpoints/cifar100_mobilenetv2_ckpt.pth'
        data_path = './data'
        ...
        pretrained_checkpoint_path = './pretrained_checkpoints/cifar10_mobilenetv2_ckpt.pth'
        data_path = './data'
    ...
    if args.resume:
        ...
        pruning_iteration = 0
        # create model
        print("=> using pre-trained model of 'MobileNet-V2'")
        base_model = MobileNetV2()
        state_dict = torch.load(pretrained_checkpoint_path)['net']
    ...
    while accuracy >= 10.5:
        ...
        if abs(acc1 - acc_tar) < 0.01:
            add_str = args.dataset + '_' + args.strategy + '_'
            save_checkpoint({
                'pruning_iteration': pruning_iteration,
                'unpruned_accuracy': unpruned_accuracy,
                'arch': args.arch,
                ...
                'state_dict': model.state_dict(),
                'acc1': acc1,
            }, is_best, filename= add_str + 'dataparallel_model.pth.tar')

            save_checkpoint({
                        'arch': args.arch,
                        'model': base_model
                    }, is_best, filename = add_str + 'base_model.pth.tar')
